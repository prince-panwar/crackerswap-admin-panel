...
// super_admin gets `warning` (amber) and admin gets `accent` (violet) so the
// two roles stay visually distinct, same as before this port (was orange vs
// violet) — just off the old retired hex values and onto the theme tokens.
const roleColors: Record<string, string> = {
  super_admin: 'bg-warning-soft text-warning border-warning-soft',
  admin: 'bg-accent-soft text-accent border-accent-soft',
};
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="relative px-5 py-4 border-b border-card-border flex items-center justify-between">
          <h3 className="text-sm font-semibold text-fg">Admin Users</h3>
          <button
            onClick={() => setShowCreate(true)}
            className="px-4 py-2 rounded-full bg-accent-soft border border-accent-soft text-accent text-xs font-semibold hover:brightness-110 transition-all cursor-pointer whitespace-nowrap"
          >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-card-border">
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-fg-subtle uppercase tracking-wider">Admin</th>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-fg-subtle uppercase tracking-wider">Role</th>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-fg-subtle uppercase tracking-wider">Status</th>
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-fg-subtle uppercase tracking-wider hidden md:table-cell">Last Login</th>
              ...
              <tbody className="divide-y divide-card-border">
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-surface transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {user.isSeeded && <p className="text-[10px] text-fg-subtle">Default admin</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 text-xs ${user.isActive ? 'text-success' : 'text-danger'}`}>
                        <i className="ri-circle-fill text-[6px]" />
                        {user.isActive ? 'Active' : 'Disabled'}
                      </span>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <span className="text-xs text-fg-tertiary">{formatDateTime(user.lastLoginAt)}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        {canManage(user) && (
                          <>
                            <button onClick={() => setChangeRoleTarget(user)} title="Change role" className="w-7 h-7 rounded-lg flex items-center justify-center text-fg-tertiary hover:text-fg hover:bg-surface transition-all cursor-pointer">
                            ...
                            <button
                              ...
                              className="w-7 h-7 rounded-lg flex items-center justify-center text-warning hover:bg-warning-soft transition-all cursor-pointer"
                            >
                              <i className={`${user.isActive ? 'ri-user-unfollow-line' : 'ri-user-follow-line'} text-sm`}></i>
                            </button>
                            <button onClick={() => setConfirmDelete(user)} title="Delete" className="w-7 h-7 rounded-lg flex items-center justify-center text-danger hover:bg-danger-soft transition-all cursor-pointer">
      ...
      */}
      ...
      {showCreate && (
        <>
          <div className="fixed inset-0 bg-overlay-backdrop backdrop-blur-sm z-[80]" onClick={() => setShowCreate(false)} />
          <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
            <div className="glass-card w-full max-w-sm rounded-[24px] overflow-hidden animate-slide-up-in">
              <div className="relative p-6 space-y-4">
                <h3 className="text-base font-semibold text-fg">Create Admin</h3>
                <div>
                  <label className="block text-xs font-semibold text-fg-secondary mb-1.5">Email</label>
                  <input
                    type="email"
                    value={newEmail}
                ...
                <div>
                  <label className="block text-xs font-semibold text-fg-secondary mb-1.5">Password (min 8 chars)</label>
                  <input
                    type="password"
                    value={newPassword}
                ...
                <div>
                  <label className="block text-xs font-semibold text-fg-secondary mb-1.5">Role</label>
                  <select
                    value={newRole}
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={() => setShowCreate(false)} className="flex-1 px-4 py-2.5 rounded-full border border-card-border text-sm text-fg-secondary hover:bg-surface transition-all cursor-pointer">Cancel</button>
                  <button onClick={handleCreate} disabled={creating} className="flex-1 px-4 py-2.5 rounded-full bg-accent-soft border border-accent-soft text-accent text-sm font-semibold hover:brightness-110 transition-all cursor-pointer disabled:opacity-50">
      ...
      {changeRoleTarget && (
        <>
          <div className="fixed inset-0 bg-overlay-backdrop backdrop-blur-sm z-[80]" onClick={() => setChangeRoleTarget(null)} />
          <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
            <div className="glass-card w-full max-w-sm rounded-[24px] overflow-hidden animate-slide-up-in">
              <div className="relative p-6">
                <h3 className="text-base font-semibold text-fg">Change Role</h3>
                <p className="text-xs text-fg-tertiary mt-1 truncate">{changeRoleTarget.email} · Current: {roleLabel(changeRoleTarget.role)}</p>
                    ...
                    <button
                      ...
                      className={`w-full px-4 py-3 rounded-xl text-left text-sm font-medium border transition-all cursor-pointer ${
                        changeRoleTarget.role === role
                          ? 'bg-cta-soft border-cta-soft text-cta-bg-active'
                          : 'bg-surface-inset border-transparent text-fg-secondary hover:border-card-border'
                      }`}
                    >
                      {roleLabel(role)}
                    </button>
                ...
                <button onClick={() => setChangeRoleTarget(null)} className="w-full mt-3 px-4 py-2.5 rounded-full border border-card-border text-sm text-fg-secondary hover:bg-surface transition-all cursor-pointer">
                  Cancel
                </button>
              </div>
            </div>
